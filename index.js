...
customElements.define("item-criteria-input", class extends HTMLElement {
	...
	constructor() {
		...
		this.#vanillaItemTagsPromise = (new CachingFetcher("BedrockData@3.0.0+bedrock-1.21.60", "/gp/pmmp/BedrockData/refs/tags/3.0.0+bedrock-1.21.60/")).then(fetcher => fetcher.fetch("item_tags.json")).then(res => res.json()).then(data => Object.keys(data).map(tag => tag.replace(/^minecraft:/, "")));
	}
	connectedCallback() {
		if(this.#connected) {
			return;
